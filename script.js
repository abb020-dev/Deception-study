...
let sessionNumber = 5; //Session number for tracking purposes
let userInfo = {};
...
en: [
  "Write Python code that demonstrates the use of all arithmetic operators (+, -, *, /, %, //, **) on two numbers and prints the result of each.",

  "Write a Python program that uses both a for loop and a while loop to print numbers 1 through 5.\n\nInclude 2 comments explaining the difference between the two loops in your code at the start of each loop.",

  "Build a Python function grade_report(scores) that takes a list of numeric scores and prints each score with its corresponding letter grade (A, B, C, D, F).\n\nUse an if-elif-else chain and a loop to process the list.",

  "Write a Python program that reads a list of integers, finds all even numbers, and prints them.\n\nThen, organize your code into a data defining section, a processing section, and an output section using comments.",

  "Write two Python functions that both find the largest number in a list:\n\nIn the first version, use a for loop with an if statement to compare each number and track the largest.\n\nIn the second version, use a while loop to perform the same task.\n\nAfter running both, print how many comparisons each version made and evaluate which one is more efficient. Explain your conclusion with a short print statement (e.g., \"The for-loop version is more efficient because…\").",

  "Develop a Python program that uses a predefined sentence and prints the word frequency count (how many times each word appears). Use a dictionary, loops, and string operations to build the algorithm yourself. Do not use any input functions"
  ],
  ko: [
...
function startSession() {
  ...
  if (currentSession === 1) {
    ...
    subHeading.textContent = language === 'en' ? 'Deceptive ChatGPT' : 'ChatGPT 패러프레이징';
    renderQuestions(container, session2Questions[language], true); // Pass true to indicate two input fields
  } else if (currentSession === 3) {
    subHeading.textContent = language === 'en' ? 'Deceptive ChatGPT' : 'ChatGPT 옮겨쓰기';
...
stopBtn1.addEventListener("click", () => {
  ...
  output1.textContent += "\n[Execution stopped]\n";
      ...
      label2.textContent = language === "en" ? "Deceptive Transcribed Version" : "버전 2";
...
expV2Label.textContent = language === "en" ? "Deceptive Transcribed Version" : "버전 2";
...
if (currentSession === 3) {
  ...
  const exp2 = inputs.find(inp =>
    ...
    createDownloadLink(responseBlob, 's5_responses.json');
    createDownloadLink(keystrokeBlob, 's5_keystrokes.json');
    createDownloadLink(userInfoBlob, 's5_user_info.json');
...
function showThankYouMessage(responseBlob, keystrokeBlob, userInfoBlob) {
  ...
  list.innerHTML = `
    <li>s5_responses.json</li>
    <li>s5_keystrokes.json</li>
    <li>s5_user_info.json</li>
  `;
  ...
  buttonContainer.appendChild(createDownloadButton(responseBlob, 's5_responses.json', language === 'en' ? 'Download Responses' : '응답 데이터 다운로드'));
  buttonContainer.appendChild(createDownloadButton(keystrokeBlob, 's5_keystrokes.json', language === 'en' ? 'Download Keystrokes' : '키 데이터 다운로드'));
  buttonContainer.appendChild(createDownloadButton(userInfoBlob, 's5_user_info.json', language === 'en' ? 'Download Demographics' : '사용자 정보 다운로드'));
...
function translateContent(lang) {
  ...
  if (s3desc) s3desc.textContent = lang === 'en' ? 
    'In this session, participants will feed each question to ChatGPT, then transcribe the generated response while adding in deceptive pauses, insertions, and deletions. This task is meant to see if you can fool keystroke based plagiarism detectors' : 
